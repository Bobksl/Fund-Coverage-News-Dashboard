...
def run(data_dir, rules, *, retrieve, post=None, max_cards=100, now=None):
    ...
    def settled(card):
        ...
        return entry is not None and entry.get('card_sha256') == news_events.card_hash(card)

    todo = [c for c in cards if not (c.get('assessment') or {}).get('assessable')
            and 'news.google.com' not in c['source']['url'] and not settled(c)]
    summarizer = summarize.Summarizer(rules, post=post, max_calls=2 * max_cards)
